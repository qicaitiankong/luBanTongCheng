...
int const ZWMSegmentHeight = TAKE_ORDER_SEGUMENT_HEIGHT;//可根据项目需求设置高度
...
- (void)segmentBasicSetting {
    ...
    _minItemSpace = 40.;
    _segmentTintColor = [UIColor whiteColor];
    _segmentNormalColor = SPECIAL_BLUE_COLOR;
    _normalBackgroundColor = [UIColor whiteColor];
    _selectedBackgroundColor = SPECIAL_BLUE_COLOR;
    _font = [UIFont getPingFangSCMedium:14];
...
//
    ...
    for (int i = 0; i < _titles.count; i++) {
        ...
        button.frame = CGRectMake(item_x, 0, _buttonSpace * 2 + titleSize.width, _segmentHeight * 0.8);
        button.center = CGPointMake(button.centerX, _segmentHeight / 2);
        [button setTag:i];
...
- (void)didClickButton:(UIButton *)button {
    ...
    [self dealDifferentBackGroundColor];
}
//lzh 处理按钮背景
- (void)dealDifferentBackGroundColor{
    for (UIButton *bt in _buttons){
        if (bt == _selectedButton){
            [bt setBackgroundColor:_selectedBackgroundColor];
        }else{
            [bt setBackgroundColor:_normalBackgroundColor];
        }
    }
}
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (weakSelf.style == ZWMSegmentStyleDefault) {
