...
- (void)scrollContainerViewToIndex:(NSUInteger)index {
    ...
    [UIView animateWithDuration:_segmentView.duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        [self->_containerView setContentOffset:CGPointMake(index * self->_size.width, 0)];
    } completion:^(BOOL finished) {
        if (weakSelf.indexBlock) {
            weakSelf.indexBlock(index, self->_segmentView.selectedButton, self.currentViewController);
        }
    }];
}
...
- (void)enumerateBadges:(NSArray<NSNumber *> *)badges {
    [badges enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [button addNumberBadge:obj.integerValue badgeOffsetSize:self->_offsetSize color:self->_segmentView.segmentTintColor borderColor:self->_segmentView.backgroundColor];
    }];
}
